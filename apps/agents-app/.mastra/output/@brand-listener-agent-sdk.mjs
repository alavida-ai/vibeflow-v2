...
var VIBEFLOW_BASE_URL = process.env.VIBEFLOW_BASE_URL || "http://localhost:4111/";
var startWorkflowResultSchema = zod_default.object({
  runId: zod_default.string().optional(),
  suspendPayload: zod_default.any().optional(),
  status: zod_default.enum(["suspended"]).optional()
});
...
var getNextStepResultSchema = external_exports.object({
  suspendPayload: external_exports.any().optional(),
  status: external_exports.enum(["suspended", "success"]).optional()
});
var VIBEFLOW_BASE_URL2 = process.env.VIBEFLOW_BASE_URL || "http://localhost:4111/";
async function getNextStep({
  ...
  try {
    const vibeflowAgentClient = new VibeflowAgentClient(VIBEFLOW_BASE_URL2);
    const mastraClient = await vibeflowAgentClient.createMastraClient();
    const workflow = mastraClient.getWorkflow(workflowId);
...
var VIBEFLOW_BASE_URL3 = process.env.VIBEFLOW_BASE_URL || "http://localhost:4111/";
var listWorkflows = async () => {
  const vibeflowAgentClient = new VibeflowAgentClient(VIBEFLOW_BASE_URL3);
  const mastraClient = await vibeflowAgentClient.createMastraClient();
